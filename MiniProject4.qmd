...
for (list in neigh_list){
  ...
  list <- gsub("[.$0-9,]", "", list)
  # Replace dashed in neighborhood names with spaces instead
  list <- gsub("[-/]", " ", list)
...
```
## Manipulate names for communities in police force dataframe.

```{r}
#glimpse(police_force)
# Keep every neighborhood listed in mn_comm
# What kind of join for this one?
...
# remove commas, dots, and dashes because of my dataframe (just merging)
# group by community because that's what I was originally interested in.
police_force |>
  distinct(police_force$Neighborho)
